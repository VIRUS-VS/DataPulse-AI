...
function App() {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setUploadMessage("Database connected successfully!");
  ...
  const handleGenerateSQL = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.detail || "Error generating SQL");
      }
    } catch (err) {
  ...
  const handleExecuteSQL = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.detail || "Error executing SQL query");
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50 p-8 font-sans text-gray-800">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <header className="text-center space-y-2">
          <h1 className="text-4xl font-extrabold text-blue-600">DataPulse AI</h1>
          <p className="text-gray-500">The living heartbeat of your data. Chat with your spreadsheets instantly.</p>
        </header>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md border border-gray-100">
          <h2 className="text-xl font-bold mb-4 border-b pb-2">1. Connect Your Data</h2>
          <form onSubmit={handleFileUpload} className="flex items-center gap-4">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-md transition-colors disabled:opacity-50 whitespace-nowrap"
            >
              {loading ? "Uploading..." : "Upload & Sync"}
            </button>
          </form>
          {uploadMessage && <p className="text-green-600 mt-3 text-sm font-medium">{uploadMessage}</p>}
          {columns.length > 0 && (
        ...
        {/* Section 2: AI Query Generation */}
        <div className="bg-white p-6 rounded-lg shadow-md border border-gray-100">
          <h2 className="text-xl font-bold mb-4 border-b pb-2">2. Ask DataPulse AI</h2>
          <div className="space-y-4">
            <textarea
              value={userPrompt}
              onChange={(e) => setUserPrompt(e.target.value)}
              placeholder="e.g., Show me the top 5 highest sales..."
              className="w-full border rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[100px]"
            ></textarea>
            <button 
        ...
        {/* Section 3: SQL Execution & Results */}
        {sqlQuery && (
          <div className="bg-white p-6 rounded-lg shadow-md border border-gray-100 space-y-4">
            <h2 className="text-xl font-bold mb-4 border-b pb-2">3. Generated Query & Results</h2>
            
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Generated SQLite Query (Editable):</label>
              <textarea
                value={sqlQuery}
                onChange={(e) => setSqlQuery(e.target.value)}
                className="w-full border rounded-md p-3 font-mono text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-500 min-h-[100px]"
              ></textarea>
            </div>
            ...
            <button 
              ...
              disabled={loading}
              className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-md transition-colors disabled:opacity-50"
            >
            ...
            {queryResults.length === 0 && !loading && sqlQuery && (
              <p className="text-gray-500 text-sm mt-4">Run the query to see results here.</p>
            )}
          </div>
        )}
      </div>
